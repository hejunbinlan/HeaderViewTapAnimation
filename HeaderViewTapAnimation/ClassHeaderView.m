...
- (void)setup {
    ...
    contentView.backgroundColor = [[UIColor yellowColor] colorWithAlphaComponent:0.1f];
...
- (void)buttonEvent:(UIButton *)button {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(tapEventWithHeaderFooterView:section:)]) {
        
        [self.delegate tapEventWithHeaderFooterView:self section:_section];
    }
}

- (void)loadData:(id)data section:(NSInteger)section {

    [super loadData:data section:section];
    
    ClassModel *model = data;
